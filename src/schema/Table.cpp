...
std::vector<TableConstraint> Table::get_table_constraints() const { return table_contraints; }
std::vector<ForeignKey> Table::get_foreign_keys() const { return foreign_keys; };
...
std::ostream& operator<<(std::ostream& os, const ForeignKey& foreign_key) {
  ...
  os << "], referenced table: " << foreign_key.referenced_table << ", referenced columns: [";
