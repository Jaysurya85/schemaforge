...
std::pair<bool, std::string> SchemaValidator::check_foreign_keys(
    const std::vector<ForeignKey>& foreign_keys, const std::vector<Table>& tables) {
  for (const auto& foreign_key : foreign_keys) {
    auto referenced_table_it =
        std::ranges::find_if(tables.begin(), tables.end(), [&foreign_key](const Table& table) {
    ...
    for (const auto& referenced_column : foreign_key.referenced_columns) {
      ...
      auto referenced_column_it = std::ranges::find_if(
          columns.begin(), columns.end(), [&referenced_column](const Column& column) {
            return column.get_column_name() == referenced_column;
          });

      if (referenced_column_it == columns.end()) {
        return {false, "Referenced column '" + referenced_column + "' not found in table '" +
